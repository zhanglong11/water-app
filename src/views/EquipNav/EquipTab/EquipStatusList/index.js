...
const EquipStatusList = (props) => {
  ...
  const renderList = () => {
    return dataList.length
      ? dataList.map((d) => {
          return (
            <View style={styles.tr} key={d.id}>
              <View style={styles.nameWrapper}>
                {d.status === 1 ? (
                  <Image
                    style={styles.img}
                    source={require(`../../../../assets/images/statusB.png`)}
                    resizeMode="contain"
                  />
                ) : (
                  <Image
                    style={styles.img}
                    source={require(`../../../../assets/images/statusR.png`)}
                    resizeMode="contain"
                  />
                )}
                <Text numberOfLines={1} style={styles.tdName}>
                  {d.name}
                </Text>
              </View>
    ...
    <View style={styles.wrapper}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.headerName} onPress={sortName}>
          ...
          <Iconfont
            style={styles.icon}
            name={nameSort ? 'arrowup' : 'arrowdown-copy'}
            size={16}
          />
        </TouchableOpacity>
        <TouchableOpacity style={styles.headerTotal} onPress={sortTotal}>
          ...
          <Iconfont
            style={styles.icon}
            name={totalSort ? 'arrowup' : 'arrowdown-copy'}
            size={16}
          />
        </TouchableOpacity>
        <TouchableOpacity style={styles.headerUnit} onPress={sortUnit}>
          ...
          <Iconfont
            style={styles.icon}
            name={unitSort ? 'arrowup' : 'arrowdown-copy'}
            size={16}
          />
        </TouchableOpacity>
      </View>
      <ScrollView style={{flex: 1}}>{renderList()}</ScrollView>
    </View>
  );
};
...
export default EquipStatusList;
