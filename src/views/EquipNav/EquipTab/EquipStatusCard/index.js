...
const EquipStatusCard = (props) => {
  ...
  const renderThreeItem = () => {
    return threeData.map((item, i) => (
      <View style={styles.list}>{renderData(item, i)}</View>
    ));
  };
  ...
  const showChart = (listIndex, i, d) => {
    if (listIndex === chartIndex && i === itemIndex) {
      ...
      setChartIndex(
        chartIndex !== -1 && chartIndex != null && listIndex > chartIndex
          ? listIndex - 1
          : listIndex,
      );
      setItemIndex(i);
        ...
        return (
          <TouchableOpacity
            ...
            {d.status === 1 ? (
              ...
              <Image
                style={styles.img}
                source={require(`../../../../assets/images/statusG.png`)}
                resizeMode="contain"
              />
            )}
  ...
  return <ScrollView style={styles.wrapper}>{renderThreeItem()}</ScrollView>;
};
...
export default EquipStatusCard;
