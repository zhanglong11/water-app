...
const EquipNav = (props) => {
  ...
  const initData = () => {
    ...
    dataArray.map((d) => {
      if (Object.keys(detailData).includes(d.id)) {
        d.showDetail = d.pointConfigList.filter((de) =>
          detailData[d.id].includes(de.id),
        );
      } else {
  ...
  const tabTitle = [{title: '项目导航'}, {title: '项目地图'}];
      ...
      <TouchableOpacity
        ...
        <View style={styles.card} key={l.projectId}>
          ...
          <View style={styles.list}>
            ...
            {l.showDetail
              ? l.showDetail.map((d, i) => (
                  <View style={styles.item} key={i}>
                    <View
                      style={{
                        ...styles.status,
                        ...{backgroundColor: status[d.status]},
                      }}
                    />
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <TabBasic tabs={tabTitle} onChange={changeTab}>
        <FlatList
          style={{flex: 1}}
          data={showData}
